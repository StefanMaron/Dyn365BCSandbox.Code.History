...
#pragma warning restore AA0074
    ...
    begin
        ...
        PassedServiceHeader.TestField("Operation Occurred Date", ErrorInfo.Create());
        PassedServiceHeader.TestField("Operation Type", ErrorInfo.Create());
        ...
        if PassedServiceHeader."Applies-to Doc. No." <> '' then
            if PassedServiceHeader."Applies-to Occurrence No." = 0 then
                Error(
                    ErrorInfo.Create(
                        StrSubstNo(Text1130004, PassedServiceHeader.FieldCaption(PassedServiceHeader."Applies-to Occurrence No.")),
                        true,
                        PassedServiceHeader));
    ...
    begin
        ...
        if TempInclInVATReportErrorLog.FindFirst() then begin
            if TempInclInVATReportErrorLog."Line No." <> 0 then
                Error(
                    ErrorInfo.Create(
                        TempInclInVATReportErrorLog."Error Message" + ' ' + StrSubstNo(Text12100, TempInclInVATReportErrorLog."Line No."),
                        true,
                        ServiceHeaderParam));

            Error(ErrorInfo.Create(TempInclInVATReportErrorLog."Error Message", true, ServiceHeaderParam));
        end;
    end;
