...
codeunit 5854 "Invt. Doc. Line-Reserve"
{
    ...
    begin
        if SourceRecRef.Number = Database::"Invt. Document Line" then
    ...
    begin
        ...
        InvtDocLine.SetReservationEntry(ReservEntry);
        sender.UpdateReservation((CreateReservEntry.SignFactor(ReservEntry) * InvtDocLine."Quantity (Base)") < 0);
    end;
    ...
    begin
        if ReservationEntry."Source Type" = Database::"Invt. Document Line" then
            if ReservationEntry."Source Subtype" = 0 then // Receipt
                Sign := 1
            else
                Sign := -1;
    end;
