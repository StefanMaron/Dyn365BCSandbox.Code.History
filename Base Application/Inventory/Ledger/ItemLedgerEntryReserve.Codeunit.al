...
codeunit 99000841 "Item Ledger Entry-Reserve"
{
    ...
    local procedure ReservationManagementOnSetReservSource(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; Positive: Boolean)
    begin
        if MatchThisTable(SourceRecRef.Number) then
            SetSourceForReservation(SourceRecRef, ReservEntry, sender, Positive);
    end;

    local procedure SetSourceForReservation(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; Positive: Boolean)
    var
        ItemLedgerEntry: Record "Item Ledger Entry";
    begin
        SourceRecRef.SetTable(ItemLedgerEntry);
