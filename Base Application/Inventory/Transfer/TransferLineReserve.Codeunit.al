...
codeunit 99000836 "Transfer Line-Reserve"
{
    ...
    local procedure ReservationManagementOnSetReservSource(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; Direction: Enum "Transfer Direction")
    begin
        if MatchThisTable(SourceRecRef.Number) then
            SetSourceForReservation(SourceRecRef, ReservEntry, sender, Direction);
    end;

    local procedure SetSourceForReservation(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; Direction: Enum "Transfer Direction")
    var
        TransferLine: Record "Transfer Line";
    begin
        SourceRecRef.SetTable(TransferLine);
        TransferLine.SetReservationEntry(ReservEntry, Direction);
        OnSetSourceForReservationOnBeforeUpdateReservation(ReservEntry, TransferLine);
#if not CLEAN25
        sender.RunOnSetTransLineOnBeforeUpdateReservation(ReservEntry, TransferLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(ReservEntry) * TransferLine."Outstanding Qty. (Base)") <= 0);
    end;
    ...
    begin
        if MatchThisTable(ReservationEntry."Source Type") then
            if ReservationEntry."Source Subtype" = 0 then // Outbound
                Sign := -1
            else
                Sign := 1;
    end;
