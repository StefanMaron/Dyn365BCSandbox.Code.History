...
codeunit 99000834 "Purch. Line-Reserve"
{
    ...
    begin
        ...
        OnSetSourceForReservationOnBeforeUpdateReservation(CalcReservEntry, PurchLine);
#if not CLEAN25
        sender.RunOnSetPurchLineOnBeforeUpdateReservation(CalcReservEntry, PurchLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * PurchLine."Outstanding Qty. (Base)") < 0);
    end;
    ...
    begin
        if MatchThisTable(ReservationEntry."Source Type") then
            if ReservationEntry."Source Subtype" in [3, 5] then // Credit memo, Return Order = demand
                Sign := -1
            else
                Sign := 1;
    end;
