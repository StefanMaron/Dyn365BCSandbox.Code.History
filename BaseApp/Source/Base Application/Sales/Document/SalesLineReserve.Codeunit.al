...
codeunit 99000832 "Sales Line-Reserve"
{
    ...
    begin
        ...
        OnSetSourceForReservationOnBeforeUpdateReservation(CalcReservEntry, SalesLine);
#if not CLEAN25
        sender.RunOnSetSalesLineOnBeforeUpdateReservation(CalcReservEntry, SalesLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * SalesLine."Outstanding Qty. (Base)") <= 0);
        OnAfterSetSourceForReservation(CalcReservEntry, SalesLine);
#if not CLEAN25
        sender.RunOnAfterSetSourceForSalesLine(CalcReservEntry, SalesLine);
#endif
    end;
