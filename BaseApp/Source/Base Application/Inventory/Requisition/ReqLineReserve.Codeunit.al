...
#pragma warning restore AA0074
    ...
    local procedure ReservationManagementOnSetReservSource(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; var RefOrderType: Enum "Requisition Ref. Order Type"; var PlanningLineOrigin: Enum "Planning Line Origin Type")
    begin
        if MatchThisTable(SourceRecRef.Number) then
            SetSourceForReservation(SourceRecRef, ReservEntry, sender, RefOrderType, PlanningLineOrigin);
    end;

    local procedure SetSourceForReservation(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management"; var RefOrderType: Enum "Requisition Ref. Order Type"; var PlanningLineOrigin: Enum "Planning Line Origin Type")
    var
        ReqLine: Record "Requisition Line";
    begin
        SourceRecRef.SetTable(ReqLine);
        ...
        OnSetSourceForReservationOnBeforeUpdateReservation(ReservEntry, ReqLine);
#if not CLEAN25
        sender.RunOnSetReqLineOnBeforeUpdateReservation(ReservEntry, ReqLine);
#endif
        sender.UpdateReservation(ReqLine."Net Quantity (Base)" < 0);
    end;
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Create Reserv. Entry", 'OnBeforeSignFactor', '', false, false)]
    local procedure GetSignfactor(var ReservationEntry: Record "Reservation Entry"; var Sign: Integer)
    begin
        if MatchThisTable(ReservationEntry."Source Type") then
            if ReservationEntry."Source Subtype" = 1 then
                Sign := -1
            else
                Sign := 1;
    end;
