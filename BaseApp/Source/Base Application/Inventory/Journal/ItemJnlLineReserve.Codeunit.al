...
codeunit 99000835 "Item Jnl. Line-Reserve"
{
    ...
    local procedure ReservationManagementOnSetReservSource(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management")
    begin
        if MatchThisTable(SourceRecRef.Number) then
            SetSourceForReservation(SourceRecRef, ReservEntry, sender);
    end;

    local procedure SetSourceForReservation(SourceRecRef: RecordRef; var CalcReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management")
    var
        ItemJnlLine: Record "Item Journal Line";
    begin
        SourceRecRef.SetTable(ItemJnlLine);
        ItemJnlLine.SetReservationEntry(CalcReservEntry);
        OnSetSourceForReservationOnBeforeUpdateReservation(CalcReservEntry, ItemJnlLine);
#if not CLEAN25
        sender.RunOnSetItemJnlLineOnBeforeUpdateReservation(CalcReservEntry, ItemJnlLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * ItemJnlLine."Quantity (Base)") < 0);
    end;
    ...
    local procedure GetSignfactor(var ReservationEntry: Record "Reservation Entry"; Inbound: Boolean; var Sign: Integer)
    begin
        if MatchThisTable(ReservationEntry."Source Type") then
            if (ReservationEntry."Source Subtype" = 4) and Inbound then
                Sign := 1
            else
                if ReservationEntry."Source Subtype" in [1, 3, 4, 5] then // Sale, Negative Adjmt., Transfer, Consumption
                    Sign := -1
                else
                    Sign := 1;
    end;
