...
#pragma warning restore AA0074
    ...
    local procedure SetSourceForReservation(SourceRecRef: RecordRef; var ReservEntry: Record "Reservation Entry"; var sender: Codeunit "Reservation Management")
    var
        JobJournalLine: Record "Job Journal Line";
    begin
        SourceRecRef.SetTable(JobJournalLine);
        JobJournalLine.SetReservationEntry(ReservEntry);
        OnSetSourceForReservationOnBeforeUpdateReservation(ReservEntry, JobJournalLine);
#if not CLEAN25
        sender.RunOnSetJobJnlLineOnBeforeUpdateReservation(ReservEntry, JobJournalLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(ReservEntry) * JobJournalLine."Quantity (Base)") < 0);
    end;
