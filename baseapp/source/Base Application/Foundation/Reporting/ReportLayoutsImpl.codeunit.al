...
codeunit 9660 "Report Layouts Impl."
{
    ...
    var
        ...
        CustomDimensions: Dictionary of [Text, Text];
    begin
        MediaOutStream := SourceTempBlob.CreateOutStream();
        ...
        if UpdateOnOnexport then begin
            LayoutSourceStream := SourceTempBlob.CreateInStream();
            LayoutTargetStream := TargetTempBlob.CreateInStream();
            if Report.ValidateAndPrepareLayout(SelectedReportLayoutList."Report ID", LayoutSourceStream, LayoutTargetStream, GetLayoutType(SelectedReportLayoutList)) then
                exit(FileManagement.BLOBExport(TargetTempBlob, FileName, true))
            else
