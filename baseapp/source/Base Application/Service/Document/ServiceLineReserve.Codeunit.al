...
codeunit 99000842 "Service Line-Reserve"
{
    ...
    begin
        ...
        OnSetSourceForReservationOnBeforeUpdateReservation(CalcReservEntry, ServiceLine);
#if not CLEAN25
        sender.RunOnSetServLineOnBeforeUpdateReservation(CalcReservEntry, ServiceLine);
#endif
        sender.UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * ServiceLine."Outstanding Qty. (Base)") <= 0);
    end;
    ...
    begin
        if MatchThisTable(ReservationEntry."Source Type") then
            if ReservationEntry."Source Subtype" in [3] then // Credit memo
                Sign := 1
            else
                Sign := -1;
    end;
